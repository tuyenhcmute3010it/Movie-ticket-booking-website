...
function convertToGMT7(date) {
  ...
  utcDate.setHours(utcDate.getHours() + 0); // Add 7 hours to UTC to get GMT+7
  return utcDate;
}
...
showMoviesForDay(0);
// ////////////////////////////////
// const movieSchedules = Array.from({ length: 14 }, (_, dayIndex) => {
//   return [
//     {
//       title: `Movie ${dayIndex + 1}`,
//       subtitle: `Subtitle for Movie ${dayIndex + 1}`,
//       description: `Description for Movie ${dayIndex + 1}`,
//       showtimes: [
//         {
//           time: "10:00 AM",
...
// function formatDate(date) {
//   const options = { weekday: "short", month: "short", day: "numeric" };
//   const localDate = new Date(date); // Convert to GMT+7 for correct display
//   return localDate.toLocaleDateString("en-US", options);
// }

// // Create tabs for the next 14 days
// function createTabs() {
//   const tabsContainer = document.querySelector(".movie-showtime__tabs");
...
//     const tabButton = document.createElement("button");
//     tabButton.classList.add("movie-showtime__tab-button");
//     tabButton.textContent = formatDate(futureDate);
//     tabButton.dataset.day = futureDate.toISOString().split("T")[0]; // Store day in ISO format
...
// function setActiveTab(dayIndex) {
//   activeTabIndex = dayIndex;
//   const tabButtons = document.querySelectorAll(".movie-showtime__tab-button");
//   tabButtons.forEach((button, index) => {
//     button.classList.toggle("active", index === dayIndex);
//   });
//   showMoviesForDay(dayIndex); // Display movies for the selected day
...
// // Display movies for the selected day
// function showMoviesForDay(dayIndex) {
//   const movieListContainer = document.getElementById("movie-schedule-list");
//   const noScheduleMessage = document.getElementById("no-schedule-message");

//   const moviesForDay = movieSchedules[dayIndex];
//   movieListContainer.innerHTML = "";
//   noScheduleMessage.textContent = "";

//   if (moviesForDay.length === 0 || !moviesForDay[0].showtimes.length) {
//     noScheduleMessage.textContent = "No sessions available for this day.";
//   } else {
//     moviesForDay.forEach((movie) => {
//       const movieElement = document.createElement("div");
//       movieElement.classList.add("movie-schedule-item");
//       movieElement.innerHTML = `
//         <h3>${movie.title}</h3>
//         <p>${movie.subtitle}</p>
//         <p>${movie.description}</p>
...
// // Initialize the tabs and the showtimes
// createTabs();
